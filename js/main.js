...
// read
// http://localhost:8000/users?=SomeTitle%Category=SomeCategory&_page=10&_limit=3
...
async function addProductDataForm(e) {
	let productId = e.target.id.split("-")[1];
...
function addEditEvent() {
	...
	editPRoductBtn.forEach(item => item.addEventListener("click", addProductDataForm));
}
...
async function saveChanges(e) {
	...
	await fetch(`${PRODUCT_API}/${e.target.id}`, {
		method: "PUT",
		body: stringify(updatedProductObj),
		headers: {
			"Content-Type": "application/json; charset=utf-8",
		},
	});
	...
	saveChanges.removeAttribute("id");
...
async function getPagesCount(product) {
	let res = await fetch(`{PRODUCT_API}`);
	let products = await res.json();

	let pagesCount = +Math.ceil(product.length / 2);
